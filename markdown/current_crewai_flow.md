...
*   **Confidence Score:** ≥ 70% (weighted calculation)
*   **Data Freshness:** ≤ 90 days old
*   **Sufficient Volume:** Based on at least 2 historical incidents
*   **Weighting Factors:**
    - Data Reliability (40%): Based on age, volume, and pattern clarity
    - Data Volume (20%): Number of historical incidents
    - Data Freshness (20%): Time since last KB update
    - Pattern Clarity (20%): Strength of false positive or success patterns
...
*   **Role:** Provides the "AI Analysis" input for the Tiered Decision Framework (advisory only).
*   **Context:** Receives the alert data and a rich summary from the **`KnowledgeQuery`** tool, which includes:
    - Data freshness with TTL tracking
    - Reliability scores (0.0-1.0)
    - Historical patterns and false positive rates
    - Resolution success metrics
    - Business hours impact analysis
*   **Task:** "Analyze the alert and provide a detailed escalation recommendation that will be used as advisory input by the decision framework."
*   **Output:** A detailed analysis object fed into the Tiered Decision Framework as optional context (`ai_analysis`), never as the primary decision factor.
...
This formula is directly implemented in the `_calculate_kb_confidence` method:
```python
def _calculate_kb_confidence(self, metrics: KnowledgeMetrics, kb_analysis: Dict, ai_analysis: Optional[str]) -> float:
    # Data reliability (40%)
    confidence = metrics.reliability_score * 0.4
    
    # Data volume (20%)
    volume_score = 1.0 if metrics.incident_count >= 10 else \
                  0.8 if metrics.incident_count >= 5 else \
                  0.6 if metrics.incident_count >= 2 else 0.2
    confidence += volume_score * 0.2
    
    # Data freshness (20%)
    freshness_score = 1.0 if metrics.data_age_days <= 30 else \
                      0.8 if metrics.data_age_days <= 90 else \
                      0.6 if metrics.data_age_days <= 180 else 0.2
    confidence += freshness_score * 0.2
    
    # Pattern clarity (20%)
    pattern_score = 1.0 if metrics.false_positive_rate >= 0.7 or metrics.false_positive_rate <= 0.3 else \
                   0.8 if metrics.false_positive_rate >= 0.6 or metrics.false_positive_rate <= 0.4 else 0.6
